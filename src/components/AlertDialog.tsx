    ...
    <Dialog role="alertdialog" {...props}>
      {({ close }) => (
        <>
          ...
          <div className={`w-6 h-6 absolute right-6 top-6 stroke-2 ${variant === 'destructive' ? 'text-error-500' : 'text-purple-500'}`}>
            {variant === 'destructive' ? <AlertCircleIcon aria-hidden /> : <InfoIcon aria-hidden />}
          </div>
          <p className="mt-3 text-neutral-500 dark:text-secondary-400">
            {children}
          </p>
